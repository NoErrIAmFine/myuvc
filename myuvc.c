...
static struct myuvc_queueueue myuvc_queue;
...
static unsigned int my_uvc_poll(struct file *file, struct poll_table_struct *wait)
{
    ...
    buf = list_first_entry(&myuvc_queue.mainqueue, struct my_uvc_buffer, stream);
...
static int my_uvc_vidioc_reqbufs(struct file *file, void *priv, struct v4l2_requestbuffers *p)
{
    ...
    if (myuvc_queue.mem)    //如果原来分配了buf,先释放原来的buf
    {
    ...
    memset(&myuvc_queue, 0, sizeof(myuvc_queue)); //清空myuvc_queue,初始化

    INIT_LIST_HEAD(&myuvc_queue.mainqueue); //初始化两个队列,my_uvc_vidioc_qbuf
    INIT_LIST_HEAD(&myuvc_queue.irqqueue);

    for (i = 0; i < nbuffers; ++i){
        myuvc_queue.buffer[i].buf.index    = i;  //索引
        myuvc_queue.buffer[i].buf.m.offset = i * bufsize; //偏移
        myuvc_queue.buffer[i].buf.length   = my_uvc_format.fmt.pix.sizeimage; //原始大小;实测PAGE_ALIGN对齐,也没问题
        ...
        myuvc_queue.buffer[i].state        = VIDEOBUF_IDLE; //分配完更新状态为空闲
...
static int my_uvc_vidioc_querybuf(struct file *file, void *priv, struct v4l2_buffer *v4l2_buf)
{
    ...
    if (myuvc_queue.buffer[v4l2_buf->index].vma_use_count) //更新flags
        v4l2_buf->flags |= V4L2_BUF_FLAG_MAPPED;

#if 0
    switch (myuvc_queue.buffer[v4l2_buf->index].state)    //将uvc flags转换成V4L2 flags
    {
        case VIDEOBUF_ERROR:
        case VIDEOBUF_DONE:
...
static int my_uvc_vidioc_qbuf(struct file *file, void *priv, struct v4l2_buffer *v4l2_buf)
{
    ...
    if (myuvc_queue.buffer[v4l2_buf->index].state != VIDEOBUF_IDLE)
        return -EINVAL;

    /* 1. 修改状态 */
    myuvc_queue.buffer[v4l2_buf->index].state = VIDEOBUF_QUEUED;
    myuvc_queue.buffer[v4l2_buf->index].buf.bytesused = 0;

    /* 2. 放入2个队列 */
    //队列1: 供应用层使用
    //当队列中缓冲区有数据时, 应用层从mainqueue队列中取出数据
    list_add_tail(&myuvc_queue.buffer[v4l2_buf->index].stream, &myuvc_queue.mainqueue);

    //队列2: 供产生数据的函数使用
    //当采集到数据时,从irqqueue队列中取出首个缓冲区,存入数据
    list_add_tail(&myuvc_queue.buffer[v4l2_buf->index].irq, &myuvc_queue.irqqueue);
...
static int my_uvc_vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *v4l2_buf)
{
    ...
    get_buf = list_first_entry(&myuvc_queue.mainqueue, struct my_uvc_buffer, stream); //取出buf
...
static void my_uvc_video_complete(struct urb *urb)
{
    ...
    /* 从irqqueue队列中取出首个缓冲区 */
    if (!list_empty(&myuvc_queue.irqqueue)) //判断是不是空队列
        buf = list_first_entry(&myuvc_queue.irqqueue, struct my_uvc_buffer, irq);//取出首buf用于后续存放数据
    else
        buf = NULL;
    ...
    for (i = 0; i < urb->number_of_packets; ++i) //一次urb传输包含number_of_packets个子包
    {
        ...
        if ((buf->state == VIDEOBUF_DONE) || (buf->state == VIDEOBUF_ERROR))
        {
            ...
            if (!list_empty(&myuvc_queue.irqqueue))
                buf = list_first_entry(&myuvc_queue.irqqueue, struct my_uvc_buffer, irq);
            else
                buf = NULL;
        }
...
static void my_uvc_uninit_urbs(void)
{
    ...
    for (i = 0; i < MY_UVC_URBS_NUM; ++i)   
    {
        ...
        {
            usb_free_coherent(my_uvc_udev, myuvc_queue.urb_size, myuvc_queue.urb_buffer[i], myuvc_queue.urb_dma[i]);
...
static int my_uvc_alloc_init_urbs(void)
{
    ...
    size = myuvc_queue.urb_size = psize * npackets; //取整后新大小

    for (i = 0; i < MY_UVC_URBS_NUM; ++i)
    {
        /* 1.分配usb_buffers */
        myuvc_queue.urb_buffer[i] = usb_alloc_coherent(my_uvc_udev, size, GFP_KERNEL | __GFP_NOWARN, &myuvc_queue.urb_dma[i]);
        /* 2.分配urb */
        myuvc_queue.urb[i] = usb_alloc_urb(npackets, GFP_KERNEL);

        if (!myuvc_queue.urb_buffer[i] || !myuvc_queue.urb[i]) //如果分配失败
        {
            my_uvc_uninit_urbs();
...
static int my_uvc_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    if (cnt == 2)
    {
        ...
        if (NULL == my_uvc_vdev){
            printk("Faile to alloc video device\n");
            return -ENOMEM;
        }
		...
		// my_uvc_vdev->device_caps = V4L2_CAP_VIDEO_CAPTURE;
